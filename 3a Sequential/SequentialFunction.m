...
function [tSeq] = SequentialFunction(FileName,Contents,Lat,Lon,RadLat,RadLon,RadO3,StartLat,NumLat,StartLon,NumLon,StartHour,EndHour,SizeLoop)
...
for NumHour = StartHour:EndHour % loop through each hour
    ...
    t1 = toc;
    t2 = t1;
    for idx = 1:SizeLoop %size(Data2Process,1) % step through each data location to process the data
